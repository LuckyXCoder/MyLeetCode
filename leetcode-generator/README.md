...
LeetCode 题目生成工具，用于从 LeetCode CN 拉取题目数据并生成 Markdown 草稿和代码模板。
...
| `generate_markdown.py` | 根据题目详情生成 Markdown 文件与 Solution.java |
| `lc_move.py` | 将题目从 generated 复制到刷题目录 |
...
└── src/leetcode_generator/ # Python 模块
...
```bash
uv run python src/leetcode_generator/sync_problems_list.py
```
...
```bash
uv run python src/leetcode_generator/parse_problem_detail.py
```
...
```bash
uv run python src/leetcode_generator/generate_markdown.py
```
...
- `Solution.java` - Java 模板（含 package 声明）
...
```bash
alias py-lc-move='cd /path/to/leetcode-generator && uv run python src/leetcode_generator/lc_move.py'
```
